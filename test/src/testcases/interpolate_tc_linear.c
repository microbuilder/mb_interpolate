...
TEST_CASE(lin_y)
{
    ...
    TEST_ASSERT(f_is_equal(y2, 0.2072f, 1E-4F));
    ...
    TEST_ASSERT_FATAL(rc == 0);
    TEST_ASSERT(f_is_equal(y2, 0.21f, 1E-4F));
    xy1.x = 10.0f;
    xy3.x = 20.0f;
...
TEST_CASE(lin_y_arr)
{
    ...
    TEST_ASSERT_FATAL(rc == 0);
    TEST_ASSERT(f_is_equal(y, -2.25f, 1E-4F));
}
...
TEST_CASE(lin_x)
{
    ...
    TEST_ASSERT_FATAL(rc == 0);
    TEST_ASSERT(f_is_equal(x2, 13.6f, 1E-4F));
    ...
    TEST_ASSERT_FATAL(rc == 0);
    TEST_ASSERT(f_is_equal(x2, 12.5f, 1E-4F));
    xy1.y = 0.20f;
    xy3.y = 0.22f;
    ...
    rc = intpl_lin_x(&xy1, &xy3, xy1.y, &x2);
    TEST_ASSERT_FATAL(rc == 0);
    TEST_ASSERT(f_is_equal(x2, xy1.x, 1E-4F));

    /* Test 7: y2 == y3. */
    rc = intpl_lin_x(&xy1, &xy3, xy3.y, &x2);
    TEST_ASSERT_FATAL(rc == 0);
    TEST_ASSERT(f_is_equal(x2, xy3.x, 1E-4F));
}
